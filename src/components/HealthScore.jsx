...
export default function HealthScore({ logs }) {
  const score = useMemo(() => {
    ...
    if (validLogs.length === 0) {
      return { value: 0, color: 'var(--color-text-muted)', status: 'No scan data yet', description: 'Complete a field scan to calculate coastal resilience.' }
    }
    ...
    const color = { low: 'var(--color-danger)', moderate: 'var(--color-warning)', high: 'var(--color-accent)' }[dominant]
    return {
